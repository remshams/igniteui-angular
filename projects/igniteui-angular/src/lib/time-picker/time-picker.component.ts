...
import {
    ...
    IgxToggleModule,
    IgxToggleDirective,
} from "../directives/toggle/toggle.directive";
import {
    TimeDisplayFormatPipe,
    TimeInputFormatPipe,
} from "./time-picker.pipes";
...
export class IgxTimePickerComponent
    ...
    set value(value: Date) {
        if (this._isValueValid(value)) {
            ...
            if (
                this.mode === InteractionMode.DropDown &&
                this._displayValue !== dispVal
            ) {
    ...
    set format(formatValue: string) {
        ...
        this.mask =
            this._format.indexOf("tt") !== -1 ? "00:00:00 LL" : "00:00:00";
     ...
     */
    @Output()
    public onValidationFailed = new EventEmitter<
    ...
    public onClosing = new EventEmitter<
        CancelableBrowserEventArgs & IBaseEventArgs
    >();
     ...
     * @hidden
     */
    @ContentChild(IgxTimePickerTemplateDirective, {
    ...
    private _itemsDelta: { hours: number; minutes: number; seconds: number } = {
        hours: 1,
        minutes: 1,
        seconds: 1,
    };
    ...
    get showHoursList(): boolean {
        return (
            this.format.indexOf("h") !== -1 || this.format.indexOf("H") !== -1
        );
    }
    ...
    get template(): TemplateRef<any> {
        ...
        return this.mode === InteractionMode.Dialog
            ? this.defaultTimePickerTemplate
            : this.dropdownInputTemplate;
    }
    ...
    public ngAfterViewInit(): void {
        if (this.mode === InteractionMode.DropDown && this._inputElementRef) {
            fromEvent(this._inputElementRef.nativeElement, "keydown")
                .pipe(
                ...
                .subscribe((event: KeyboardEvent) => {
                    if (
                        ...
                        event.key === KEYS.UP_ARROW_IE ||
                        event.key === KEYS.DOWN_ARROW ||
                        event.key === KEYS.DOWN_ARROW_IE
                    ) {
        ...
        if (this.toggleRef && this._inputGroup) {
            this.toggleRef.element.style.width =
                this._inputGroup.element.nativeElement.getBoundingClientRect()
                    .width + "px";
        }

        if (this.toggleRef) {
            this.toggleRef.onClosed
            ...
            this.toggleRef.onOpened
                .pipe(takeUntil(this._destroy$))
                .subscribe(() => {
                    this.onOpened.emit(this);
                });

            this.toggleRef.onClosing
                .pipe(takeUntil(this._destroy$))
                .subscribe((event) => {
                    ...
                    if (
                        input &&
                        !(event.event && this.mode === InteractionMode.DropDown)
                    ) {
                        input.focus();
                    } else {
            ...
            if (this._ngControl) {
                this._statusChanges$ = this._ngControl.statusChanges.subscribe(
    ...
    private _scrollItemIntoView(
        ...
        if (items) {
            const index =
                item === "AM" || item === "PM"
                    ? items.indexOf(item)
                    : items.indexOf(parseInt(item, 10));
            let view;

            if (index !== -1) {
                if (isListLoop) {
                    if (index > 0) {
                        selectedItem = this._itemToString(
                            items[index - 1],
                            viewType
                        );
                        itemIntoView = this._nextItem(
                            items,
                            selectedItem,
                            isListLoop,
                            viewType
                ...
                itemIntoView.view = this._viewToString(
                    itemIntoView.view,
                    viewType
                );
            }
    ...
    private _itemToString(item: any, viewType: string): string {
        if (item === null) {
            ...
            const leadZeroHour =
                item < 10 &&
                (this.format.indexOf("hh") !== -1 ||
                    this.format.indexOf("HH") !== -1);
            const leadZeroMinute =
                item < 10 && this.format.indexOf("mm") !== -1;
            const leadZeroSeconds =
                item < 10 && this.format.indexOf("ss") !== -1;
    ...
    private _prevItem(
        ...
        if (selectedIndex === -1) {
            ...
            if (selectedIndex - 4 < 0) {
                ...
                view = view.concat(
                    items.slice(0, selectedIndex + 3 - itemsCount)
                );
            } else {
    ...
    private _nextItem(
        ...
        if (selectedIndex === -1) {
            ...
            if (selectedIndex < 2) {
                ...
                view = view.concat(
                    items.slice(0, selectedIndex + 5 - itemsCount)
                );
            } else {
    ...
    private _formatTime(value: Date, format: string): string {
        if (!value) {
            ...
            formattedMinute =
                minute < 10 && format.indexOf("mm") !== -1
                    ? "0" + minute
                    : `${minute}`;

            formattedSeconds =
                seconds < 10 && format.indexOf("ss") !== -1
                    ? "0" + seconds
                    : `${seconds}`;
    ...
    private _updateHourView(start: any, end: any): void {
        this._hourView = this._viewToString(
            this._hourItems.slice(start, end),
            "hour"
        );
    }

    private _updateMinuteView(start: any, end: any): void {
        this._minuteView = this._viewToString(
            this._minuteItems.slice(start, end),
            "minute"
        );
    }

    private _updateSecondsView(start: any, end: any): void {
        this._secondsView = this._viewToString(
            this._secondsItems.slice(start, end),
            "seconds"
        );
    }
    ...
    private _getSelectedTime(): Date {
        ...
        if (
            ((this.showHoursList && this.selectedHour !== "12") ||
                (!this.showHoursList && this.selectedHour <= "11")) &&
    ...
    public convertMinMaxValue(value: string): Date {
        ...
        if (this.showAmPmList) {
            ...
            if (
                ((this.showHoursList && date.getHours().toString() !== "12") ||
                    (!this.showHoursList &&
                        date.getHours().toString() <= "11")) &&
                amPM === "PM"
            ) {
                date.setHours(date.getHours() + 12);
            }
    ...
    private _isValueValid(value: Date): boolean {
        if (this.maxValue && value > this.convertMinMaxValue(this.maxValue)) {
            ...
            this.minValue &&
            value < this.convertMinMaxValue(this.minValue)
        ) {
            return false;
        } else {
            return true;
    ...
    private _isEntryValid(val: string): boolean {
        ...
        if (this.showHoursList) {
            validH =
                this.validHourEntries.indexOf(
                    parseInt(sections[0].replace(re, ""), 10)
                ) !== -1;
        }
        ...
        if (this.showMinutesList) {
            ...
            validM =
                this.validMinuteEntries.indexOf(
                    parseInt(minutes.replace(re, ""), 10)
                ) !== -1;
        }

        if (this.showSecondsList) {
            const seconds =
                sections[sections.length - (this.showAmPmList ? 2 : 1)];
            validS =
                this.validSecondsEntries.indexOf(
                    parseInt(seconds.replace(re, ""), 10)
                ) !== -1;
        }
    ...
    private _spinHours(
        ...
        if (currentVal.getTime() > maxVal.getTime()) {
            if (this.isSpinLoop) {
                minutes =
                    minutes < minVal.getMinutes() ? 60 + minutes : minutes;
            ...
            if (this.isSpinLoop) {
                minutes =
                    minutes <= maxVal.getMinutes() ? minutes : minutes - 60;
    ...
    private _initializeContainer() {
        if (this.value) {
            ...
            if (this.showMinutesList) {
                this.selectedMinute = this.showHoursList
                    ? sections[1]
                    : sections[0];
            }
        ...
        if (this.selectedHour === undefined) {
            this.selectedHour =
                !this.showHoursList && this.value
                    ? this.value.getHours().toString()
                    : this.showHoursList
                    ? `${this._hourItems[3]}`
                    : "0";
        }
        if (this.selectedMinute === undefined) {
            this.selectedMinute =
                !this.showMinutesList && this.value
                    ? this.value.getMinutes().toString()
                    : "0";
        }
        if (this.selectedSeconds === undefined) {
            this.selectedSeconds =
                !this.showSecondsList && this.value
                    ? this.value.getSeconds().toString()
                    : "0";
        }
    ...
    private _onDropDownClosed(): void {
        ...
        if (this._isValueValid(newVal)) {
            ...
            this.displayValue = this.inputFormat.transform(
                this._formatTime(oldValue, this.format)
            );
    ...
    protected onStatusChanged() {
        if (
            ...
            const input =
                this._inputDirective || this._inputDirectiveUserTemplate;
            if (this._inputGroup.isFocused) {
                input.valid = this._ngControl.valid
                    ? IgxInputState.VALID
    ...
    getEditElement() {
        return this._inputElementRef
            ? this._inputElementRef.nativeElement
            : null;
    }
    ...
    public scrollHourIntoView(item: string): void {
        if (this.showHoursList) {
            const hourIntoView = this._scrollItemIntoView(
                item,
                this._hourItems,
                this.selectedHour,
                this._isHourListLoop,
                "hour"
            );
            if (hourIntoView) {
    ...
    public scrollMinuteIntoView(item: string): void {
        if (this.showMinutesList) {
            const minuteIntoView = this._scrollItemIntoView(
                item,
                this._minuteItems,
                this.selectedMinute,
                this._isMinuteListLoop,
                "minute"
            );
            if (minuteIntoView) {
    ...
    public scrollSecondsIntoView(item: string): void {
        if (this.showSecondsList) {
            const secondsIntoView = this._scrollItemIntoView(
                item,
                this._secondsItems,
                this.selectedSeconds,
                this._isSecondsListLoop,
                "seconds"
            );
            if (secondsIntoView) {
    ...
    public scrollAmPmIntoView(item: string): void {
        if (this.showAmPmList) {
            const ampmIntoView = this._scrollItemIntoView(
                item,
                this._ampmItems,
                this.selectedAmPm,
                false,
     ...
     */
    public nextMinute() {
        const nextMinute = this._nextItem(
            this._minuteItems,
            this.selectedMinute,
            this._isMinuteListLoop,
            "minute"
     ...
     */
    public prevMinute() {
        const prevMinute = this._prevItem(
            this._minuteItems,
            this.selectedMinute,
            this._isMinuteListLoop,
            "minute"
     ...
     */
    public nextSeconds() {
        const nextSeconds = this._nextItem(
            this._secondsItems,
            this.selectedSeconds,
            this._isSecondsListLoop,
            "seconds"
     ...
     */
    public prevSeconds() {
        const prevSeconds = this._prevItem(
            this._secondsItems,
            this.selectedSeconds,
            this._isSecondsListLoop,
            "seconds"
    ...
    public cancelButtonClick(): void {
        if (this.mode === InteractionMode.DropDown) {
            this.displayValue = this.value
                ? this._formatTime(this.value, this.format)
                : this.parseMask(false);
        }
    ...
    public parseMask(preserveAmPm = true): string {
        const maskWithAmPm = this.mask
            .replace(new RegExp("0", "g"), this.promptChar)
            .replace("LL", "AM");
        const pureMask = this.mask
            .replace(new RegExp("0", "g"), this.promptChar)
            .replace(new RegExp("L", "g"), this.promptChar);
        ...
        if (this.mode === InteractionMode.DropDown) {
            ...
            if (value && value !== this.parseMask()) {
                if (this._isEntryValid(value)) {
                    ...
                    if (
                        !this.value ||
                        this.value.getTime() !== newVal.getTime()
                    ) {
                        this.value = newVal;
                    }
                } else {
    ...
    public spinOnEdit(event): void {
        ...
        const min = this.minValue
            ? this.convertMinMaxValue(this.minValue)
            : this.convertMinMaxValue("00:00");
        const max = this.maxValue
            ? this.convertMinMaxValue(this.maxValue)
            : this.convertMinMaxValue("24:00");
        ...
        if (event.key) {
            const key = event.key;
            sign =
                key === KEYS.DOWN_ARROW || key === KEYS.DOWN_ARROW_IE ? -1 : 1;
        }
        ...
        if (!this.displayValue) {
            ...
            if (
                this.cursorOnSeconds(
                    cursor,
                    this.showHoursList,
                    this.showMinutesList,
                    this.showSecondsList
                )
            ) {
    ...
    private cursorOnMinutes(
        ...
        return (
            (showMinutes && showHours && this._minutesPos.has(cursor)) ||
